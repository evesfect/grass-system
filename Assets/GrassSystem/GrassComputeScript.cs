// @evesfect version
// credits  to Minionsart https://www.patreon.com/minionsart
// for the base grass system
...
public class GrassComputeScript : MonoBehaviour
{
    ...
    public float windTextureStrength = 0.3f;
    public float windTextureSpeed = 0.05f;
    public float windContrastMin = 0.0f;
    public float windContrastMax = 0.7f;
    public float windTextureScale = 0.0065f;

    [Range(0.0f, 1.0f)]
    public float windColorStrength = 1.0f;
    [Range(-1.0f, 1.0f)]
    public float windDarkenAmount = -0.2f;
    ...
    private uint[] argsBufferReset = new uint[5]
   {
    ...
    void MainSetup(bool full)
    {
        ...
        if (windTexture == null)
        {
            Debug.LogWarning("Missing Wind Texture. Using dummy texture");
            // Create a temporary 1x1 white texture as fallback
            Texture2D dummy = new Texture2D(1, 1);
            dummy.SetPixel(0, 0, Color.white);
            dummy.Apply();
            windTexture = dummy;
        }

        m_InstantiatedComputeShader.SetTexture(m_IdGrassKernel, "_WindTexture", windTexture);
        m_InstantiatedMaterial.SetTexture("_WindTexture", windTexture);

        // Pass the wind texture strength and speed parameters.
        m_InstantiatedComputeShader.SetFloat("_WindTextureStrength", windTextureStrength);
        m_InstantiatedComputeShader.SetFloat("_WindTextureSpeed", windTextureSpeed);

        m_InstantiatedMaterial.SetFloat("_WindTextureStrength", windTextureStrength);
        m_InstantiatedMaterial.SetFloat("_WindTextureSpeed", windTextureSpeed);

        m_InstantiatedComputeShader.SetFloat("_WindContrastMin", windContrastMin);
        m_InstantiatedComputeShader.SetFloat("_WindContrastMax", windContrastMax);
        m_InstantiatedComputeShader.SetFloat("_WindTextureScale", windTextureScale);

        m_InstantiatedMaterial.SetFloat("_WindContrastMin", windContrastMin);
        m_InstantiatedMaterial.SetFloat("_WindContrastMax", windContrastMax);
        m_InstantiatedMaterial.SetFloat("_WindTextureScale", windTextureScale);

        m_InstantiatedComputeShader.SetFloat("_WindColorStrength", windColorStrength);
        m_InstantiatedComputeShader.SetFloat("_WindDarkenAmount", windDarkenAmount);
        m_InstantiatedMaterial.SetFloat("_WindColorStrength", windColorStrength);
        m_InstantiatedMaterial.SetFloat("_WindDarkenAmount", windDarkenAmount);
    ...
    private void SetGrassDataBase(bool full)
    {
        ...
        if (full)
        {
            ...
            interactors = (ShaderInteractor[])FindObjectsByType(typeof(ShaderInteractor), FindObjectsSortMode.None);
        }
        else
        {
